...
score = [[0, 15, 250, 400000, 10000000], # live
         [0, 10, 220, 250, 10000000], # run
         [0, 2, 20, 250, 10000000], # sleep
         [100000, 200000, 300000, 400000, 10000000]]# KO
...
class AI(object):
    ...
    def min_max_decision(self, chessboard, depth):
        color = self.color
        inf = float("inf")
        # 取前几个位置(每个节点的子树数控制在这几个之内)
        count_len = 4

        def max_value(chessboard, old_chess, curr_d, alpha, beta):
            if curr_d == depth:
                # 计算state的分数,,不能这样计算
        ...
        def min_value(chessboard, old_chess, curr_d, alpha, beta):
            if curr_d == depth:
                v = self.total_eval(chessboard, old_chess)
                return v
            v = inf
            new_empty = self.sort_empty(chessboard, -1*color)
            for b in new_empty[:count_len]:
                # old_chess = np.copy(chessboard)
                chessboard[b[0][0],b[0][1]] = -1*color
                if b[1] >= score[3][4]*1.05:
                    chessboard[b[0][0], b[0][1]] = 0
                    return -1*b[1]
                else:
                    v = min(v, max_value(chessboard, old_chess, curr_d+1, alpha, beta))
        ...
        # sorted_empty = [[(8, 12), 420017.75], [(9, 8), 10499965]]
        # 预先加进一个点，防止博弈树计算超时
        first_pos = sorted_empty[0][0]
        assert chessboard[first_pos[0], first_pos[1]] == COLOR_NONE
        self.candidate_list.append(first_pos)
        # 开始博弈树
        best_score = -inf
        beta = inf
        best_action = None
        ...
        for a in sorted_empty[:count_len]:
            ...
            if a[1] >= score[3][4]*1.05:
                ...
                v = min_value(chessboard, old_chess, 1, best_score, beta)
            chessboard[a[0][0], a[0][1]] = 0
            print(a)
            print(v)
            if v > best_score:
    ...
    def point_value_4_dir(self, chessboard, each, color, chess_type):
        ...
        # 必胜棋型判断
        # if chess_type[0][4] + chess_type[1][4] + chessboard[2][4]> 0:  # 连五
        #     score_all += KO[4]
        # elif chess_type[0][3] > 0:  # 活四
        #     score_all += KO[3]
        # el
        if chess_type[1][3] + chess_type[2][3] > 1:  # 双冲四
            chess_type[3][2] += 1
            score_all += score[3][2]*color
            if chess_type[1][3] > 1:
                chess_type[1][3] -= 2
    ...
    def sort_empty(self, chessboard, color):
        ...
        for each in empty:
            ...
            syn_score_me = self.point_value_4_dir(chessboard, each, color, chess_type1)*1.05
